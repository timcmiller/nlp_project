...
var afinn = JSON.parse(fs.readFileSync(__dirname + '/AFINN.json', 'utf-8'));
var enNegationWords = require(__dirname + '/neg-words');
...
var sentimentChecker = module.exports = function(text, language) {
  ...
  if(language === 'french') {
    negationWords = frNegationWrods;
    language = frSentiment;
  } else if(language === 'portuguese') {
    negationWords = ptNegationWrods;
    language = ptSentiment;
  } else if(language === 'sweedish') {
    negationWords = swNegationWrods;
    language = swSentiment;
  } else {
    negationWords = enNegationWords;
    language = afinn;
  }
  ...
  returnValue.sentimentValue = (Math.ceil(returnValue.sentimentValue * 100) / 100);
  ...
  if((returnValue.sentimentValue >= -0.4 && returnValue.sentimentValue <= 0.4) || isNaN(returnValue.sentimentValue)) {
