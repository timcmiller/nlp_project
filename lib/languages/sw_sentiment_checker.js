...
var swStopWords = fs.readFileSync(__dirname + '/sw-stop.js', 'utf-8');
var swSentiment = fs.readFileSync(__dirname + '/sw-sentiment.json', 'utf-8');
var swNegationWords = require(__dirname + '/sw-neg.js');
//add in different language lists here put them in a varible with the name of the language
swSentiment = JSON.parse(swSentiment);

var sentimentChecker = module.exports = function(text, language) {

  language = language || swSentiment;
  ...
  var counter = 0;
  for(i = 0, j = 0; i < sentimentWords.length; i++) {
